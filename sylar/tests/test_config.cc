...
sylar::ConfigVar<float>::ptr g_float_value_config = sylar::Config::Lookup("system.value", (float)10.2f, "system port");
...
void test_yaml() {
    YAML::Node root = YAML::LoadFile("/home/hades/cpp_program/sylar/bin/conf/test.yml");
...
#define XX_PM(g_var, prefix) \
    ...
    //SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_person_vec_map->toStirng();
    ...
    //SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "after: " << g_person_vec_map->toStirng();
...
void test_log() {
    ...
    YAML::Node root = YAML::LoadFile("/home/hades/cpp_program/sylar/bin/conf/log.yml");
    sylar::Config::LoadFromYaml(root);
    std::cout << "=============" <<std::endl;
    std::cout << sylar::LoggerMgr::GetInstance()->toYamlString() << std::endl;
    std::cout << "=============" <<std::endl;
